...
RBTree *create_tree(const TreeValue &tv) {
    init_nil_node();
    RBTree *rbt = (RBTree *)_MALLOC(sizeof(RBTree));
    TreeNode *root = (TreeNode *)_MALLOC(sizeof(TreeNode));
...
double find_max_gradient_within_key(RBTree *rbt, double key, double angle,
                                    double gradient) {
...
void init_nil_node() {
    NIL = (TreeNode *)_MALLOC(sizeof(TreeNode));
    NIL->color = RB_BLACK;
    NIL->value.angle[0] = 0;
    NIL->value.angle[1] = 0;
    NIL->value.angle[2] = 0;
...
char compare_values(TreeValue *v1, TreeValue *v2) {
    if (v1->gradient[1] > v2->gradient[1]) return 1;
    if (v1->gradient[1] < v2->gradient[1]) return -1;
 ...
 * a > b : 1
 * a == b : 0 */
char compare_double(double a, double b) { return (a < b ? -1 : (a > b)); }

/*create a tree node */
TreeNode *create_tree_node(const TreeValue &value) {
    TreeNode *ret;

    ret = (TreeNode *)_MALLOC(sizeof(TreeNode));
...
void delete_from_tree(TreeNode **root, double key) {
    ...
    if (z == NIL) {
        ...
        _ABORT_F(_("Attempt to delete node with key=%f failed"), key);
    }
    ...
    if (y == NIL) {
        _ABORT_F(_("Successor node not found. Deletion fails."));
    }
    ...
    while (curNode->parent != NIL) {
        if (curNode->parent->value.maxGradient ==
            find_value_min_value(&y->value)) {
    ...
    /*16-17 */
    if (y->color == RB_BLACK && x != NIL) rb_delete_fixup(root, x);
...
double find_max_value(TreeNode *root) {
    if (!root) return SMALLEST_GRADIENT;
    assert(root);
...
double find_max_value_within_key(TreeNode *root, double maxKey, double angle,
                                 double gradient) {
    ...
    if (keyNode == NIL) {
        ...
        /*node not found */
        _ABORT_F(_("Attempt to find node with key=%f failed"), maxKey);
    ...
    while (currNode->parent != NIL) {
        if (currNode ==
            currNode->parent->right) { /*its the right node of its parent; */
    ...
    while (currNode != NIL) {
        ...
        if (!checkme && currNode->value.key > 0) {
            _LOG_WARNING(_("Angles outside angle %.4f"), angle);
            _LOG_WARNING(_("ENTER angle %.4f"), currNode->value.angle[0]);
            _LOG_WARNING(_("CENTER angle %.4f"), currNode->value.angle[1]);
            _LOG_WARNING(_("EXIT angle %.4f"), currNode->value.angle[2]);
            _LOG_WARNING(_("ENTER gradient %.4f"), currNode->value.gradient[0]);
            _LOG_WARNING(_("CENTER gradient %.4f"),
                         currNode->value.gradient[1]);
            _LOG_WARNING(_("EXIT gradient %.4f"), currNode->value.gradient[2]);
        }

        if (currNode->value.key > maxKey) {
            _ABORT_F(_("current dist too large %.4f > %.4f"),
                     currNode->value.key, maxKey);
        }
        ...
        if (checkme && currNode != keyNode) {
            ...
            if (curr_gradient > max) max = curr_gradient;
        ...
        if (currNode->left != NIL) {
            ...
            while (currNode->right != NIL) currNode = currNode->right;
        } else {
    ...
    while (keyNode->parent != NIL) {
        if (keyNode ==
            keyNode->parent->right) { /*its the right node of its parent; */
...
void left_rotate(TreeNode **root, TreeNode *x) {
    ...
    tmpMax = x->left->value.maxGradient > y->left->value.maxGradient
                 ? x->left->value.maxGradient
                 : y->left->value.maxGradient;
